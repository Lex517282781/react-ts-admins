...
/** file转base64 */
export const getBase64 = function(file: File) {
...
/** base64转file */
export const dataURLtoFile = function(dataURL: string, filename: string): File {
...
// /** 图片类型判断 */
export const isPic = function (type: string): boolean {
  ...
  const isPic: boolean = isJPG || isPNG || isBMP || isGIF || isWEBP;
  return isPic
}

/** size为n n小于1的 (n*1000)kb n大于等于1 (n)mb，1000kb-1024kb区间不支持设置 */
export const sizeOverflow = function (cursize: number, presize: number): boolean {
  return (cursize / 1024 / 1024) > (presize < 1 ? presize / 1.024 : presize)
}

/** 根据图片大小获取大小M和KB值 */
export const getSizeTxt = function (size: number) {
  if (size >= 1) {
    return size + 'MB'
  } else {
    return size * 1000 + 'KB'
...
/** 获取base64尺寸 */
export const getBase64Size = function (base64: string): number {
...
/** 从文件url获取文件名 */
export const derivedNameFormUrl =function (url: string): string {
  return (url || '').replace(/(.+)\/(?=.+$)/, '')
}

/** 获取文件扩展名 */
export const getFileExtName = function(url: string) {
